...
| `BSD/EngineObligations.lean` | Engine-emitted obligations (experiment (l) VERIFY): the open frontier nodes as `sorry` theorems; `engine_rankTwoObject_obligation` corresponds to `rankTwoCertificate`. Generated by [`experiments/engine/verify.py`](../experiments/engine/verify.py). |
