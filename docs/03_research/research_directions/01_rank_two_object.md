...
The Kudla row is the live one for Clause 2: it is the only known mechanism whose output is structurally a second derivative. The surveyor's next pass should extract exactly what object plays $\mathrm{Aux}(E)$ there and run the Detector 3 audit on it.
