...
The rank-2 object is now specified ([Direction 01](docs/03_research/research_directions/01_rank_two_object.md)) and the discipline runs as the engine of experiment (l). The engine's PROPOSE battery leaves exactly one construction class lit as the live candidate: the **Kudla program / arithmetic theta series**, the only formalism with the Clause-2 second-derivative shape (codimension-$r$ arithmetic cycles against genus-$r$ Eisenstein series). This front is now specified ([Direction 04](docs/03_research/research_directions/04_kudla_clause_two_bridge.md)) with its computable rank-1 shadow built and running (experiment (m): Waldspurger's proportionality on the congruent-number family, $\kappa$ constant to $\sim 30$ digits). Direction 05 then reframed Bridge 1's ORDER GAP as an archimedean obstruction (experiment (n): the p-adic height regulator of 389a1, $v_5(\mathrm{Reg}_5) = v_7(\mathrm{Reg}_7) = 2$, so the second-order Clause-2 object is computable p-adically, though the rank-2 identity to $L_p^{(2)}$ stays conjectural), with the cyclotomic / weight axis identified as the number-field substitute for the Yun-Zhang second leg. So the most-leveraged next move is sharpened to the two obstructions that remain: Bridge 2 (point extraction, the codimension wall, since a codim-$\geq 2$ cycle has no Jacobian image and yields a height, not a point) and the archimedean transfer (the p-adic regulator to the complex leading term, the Beilinson-Bloch-Kato gap). Anything claimed here runs through Detector 3 first: the higher-derivative layer is a function-field theorem, so a transfer to $\mathbb{Q}$ must not silently import the surface, and through Detector 2: $\mathrm{Sha}$-finiteness in rank $\geq 2$ sits in the p-adic leading term, relocated not removed.
