...
- Experiment (h) gross_zagier_check (2026-06-11): the Gross-Zagier identity L'(E,1) L(E_D,1) = Vol(Lambda) hhat(P)/sqrt|D| verified with every factor computed independently: 37a1 over Q(sqrt -67) and Q(sqrt -11), 43a1 and 53a1 over Q(sqrt -163); worst |ratio - 1| = 1.1e-20; Heegner indices exactly integral (6, 1, 7, 5). New substrate: period lattice extended to Delta < 0 (rhombic, direct period integrals), and sigma-function canonical heights to ~20 digits ([`experiments/_shared/analytic_height.py`](experiments/_shared/analytic_height.py)), normalization pinned by quadraticity + three bundled regulators. Smoke test still 7/7.
...
2. **Surveyor pass on the Kudla program** (arithmetic theta series): per the spec's scorecard it is the only known systematic source of second derivatives, hence the live candidate for Clause 2; extract what plays Aux(E) and run the Detector 3 audit.
...
5. The sigma-height machinery (new in h) makes a high-precision regulator pipeline possible on the rank >= 2 curves: replace the 1e-5 doubling regulators in experiment (f) with ~20-digit values, sharpening the strong-BSD residuals in experiment (c).
