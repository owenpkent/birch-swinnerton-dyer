...
- [x] Smoke test, 11/11 passing (`smoke_test.py`; check 7 added 2026-06-09, check 8 + check 9 added 2026-06-12, check 10 + check 11 added 2026-06-13)
...
- [x] Experiment (n) `padic_regulator/` + Direction 05: a real swing at Kudla Bridge 1's order gap, adversary-gated for overclaiming. Verdict (`docs/03_research/research_directions/05_padic_second_leg.md`): the order gap is an ARCHIMEDEAN obstruction; the p-adic side has a genuine second deformation axis (cyclotomic + weight), so the second-order Clause-2 object (the p-adic height regulator) is definable and COMPUTABLE there. Surveyor downgraded the headline: proven p-adic Gross-Zagier (Perrin-Riou, Disegni) is order ONE; the rank-2 identity det Reg_p = (stuff) L_p^(2) is CONJECTURAL (p-adic BSD). Witness: new engine `_shared/padic_height.py` (from-scratch Mazur-Stein-Tate p-adic heights, all self-checks pass) computes v_5(Reg_5(389a1)) = v_7(Reg_7) = 2 (c-independent, matching the canonical example; unit digits gated behind the named missing constant c via NotImplementedError, the experiment-(k) discipline). Points are INPUT, Bridge 2 untouched. Smoke check 11. A reframing that sharpens the open problem to Bridge 2 + the archimedean transfer + Sha; NOT a proof (2026-06-13)
